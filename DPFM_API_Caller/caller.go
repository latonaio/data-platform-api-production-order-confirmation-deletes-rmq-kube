...
import (
	...
	"data-platform-api-production-order-confirmation-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	header.ProductionOrderConfirmation = input.Header.ProductionOrderConfirmation
	...
	for i := range *items {
		...
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": (*items)[i], "function": "ProductionOrderConfirmationItem", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
			return nil, nil
...
func (c *DPFMAPICaller) itemDelete(
	...
	for _, v := range input.Header.Item {
		data := dpfm_api_output_formatter.Item{
			ProductionOrderConfirmation:      input.Header.ProductionOrderConfirmation,
			ProductionOrderConfirmationItem:  v.ProductionOrderConfirmationItem,
			IsMarkedForDeletion:  v.IsMarkedForDeletion,
		}
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{
			"message":            data,
			"function":           "ProductionOrderConfirmationItem",
		...
		if !checkResult(res) {
			output.SQLUpdateResult = getBoolPtr(false)
			output.SQLUpdateError = "ProductionOrderConfirmation Item Data cannot delete"
			return nil
		}
	}
	// itemがキャンセル取り消しされた場合、headerのキャンセルも取り消す
	if !*input.ProductionOrderConfirmation.Item[0].IsMarkedForDeletion {
		header := c.HeaderRead(input, log)
